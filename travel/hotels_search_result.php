...
function Get_search_id(){
	...
	$conn = $args[2];
	$id_search = array();
	$sqlsearch = "select * from hotels where ".$queryfield." like '%".$queryvalue."%'";
...
foreach ($resultid as $key => $value) {
	$bresult = mysqli_query($conn,"select * from hotels where name='$resultid[$key]'");
	$binfo = mysqli_fetch_array($bresult);
echo "<tr align=center><td>$binfo[name]</td>";
echo "<td>$binfo[location]</td>";
echo "<td>$binfo[price]</td>";
echo "<td>$binfo[numRooms]</td>";
echo "<td>$binfo[numAvial]</td>";
echo "<td><a href='hotels_book.php' color=#0000ff>book</a></td>";
